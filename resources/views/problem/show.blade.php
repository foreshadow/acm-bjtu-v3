              ...
              <li role="presentation" class="pull-right">
                <form action="/problem/{{ $problem->id }}" method="post">
                  ...
                  <button type="submit" class="btn btn-danger"
                          data-confirm="确定要删除题目 {{ $problem->title }} 吗？">@icon('trash')删除
                  </button>
                </form>
              </li>
            @endif
    ...
    <div class="col-md-3">
      <div class="panel panel-default">
        ...
        <div class="panel-body">
          @if ($problem->generator)
            <h4>
              ...
              <div class="pull-right">
                <small>{{ Storage::size($problem->generator) }} 字节</small>
              </div>
              <div>
                <small>修改于 {{  date('Y/n/j G:i', Storage::lastModified($problem->generator)) }}</small>
              </div>
            </h4>
          ...
          @if ($problem->solution)
            <h4>
              <a href="{{ code($problem->solution) }}">{{ ucfirst(explode('/', $problem->solution)[3]) }}</a>
              <div class="pull-right">
                <small>{{ Storage::size($problem->solution) }} 字节</small>
              </div>
              <div>
                <small>修改于 {{  date('Y/n/j G:i', Storage::lastModified($problem->solution)) }}</small>
              </div>
            </h4>
